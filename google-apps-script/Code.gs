/**
 * ==============================================================================
 * KOIOSTUDIO - UNIFIED GOOGLE APPS SCRIPT WEB APP
 * Handles:
 ...
 *  4. Mahi's Circle Connections Form (/connections)
 *
 * Features:
 *  - Appends submission rows to dedicated tabs in Google Sheets
 *  - Generates a styled Google Document (.docx / Google Doc format, NOT PDF)
 *  - Emails the .docx document and Google Doc link to the Owner (Mahi)
 *  - Sends a branded confirmation email to the Client & Owner
 * ==============================================================================
 */

// CONFIGURATION: Set the owner email and drive folder name
const CONFIG = {
  OWNER_EMAIL: "mahi@koiostudio.com", // Change to Mahi's preferred email address
  OWNER_NAME: "Mahi",
...
/**
 * Handle incoming POST requests from Koiostudio forms
 */
function doPost(e) {
  try {
    ...
    // Identify form type
    const formType = data.formType || (data.fullName ? "Mahi's Circle – Trusted Connections" : "Client Onboarding");
    const ss = SpreadsheetApp.getActiveSpreadsheet();

    let docUrl = "";
    let docxBlob = null;
    let clientEmail = data.email || data.contactEmail || "";
    let clientName = data.contactPerson || data.fullName || "Valued Client";
    ...
    if (formType.includes("Ads Onboarding")) {
      ...
      // Connections Form
      handleConnectionsSheet(ss, data);
    }
...
function doGet(e) {
  return ContentService.createTextOutput(
    JSON.stringify({
      status: "online",
      service: "Koiostudio Onboarding & Form Backend",
...
function handleConnectionsSheet(ss, d) {
  const headers = [
    "Timestamp", "Full Name", "Email", "WhatsApp", "Phone", "State", "City", "Company Name",
    "Locations Served", "Services Offered", "Category", "Keywords", "Team Setup", "Target Clients",
    "Open for Referrals", "Referral Types", "Website URL", "Portfolio URL", "LinkedIn URL", "Elevator Pitch"
  ];
  const sheet = getOrCreateSheet(ss, "Mahi's Circle Connections", headers);
  sheet.appendRow([
    new Date(), d.fullName || "", d.email || "", d.whatsapp || "", d.phone || "", d.state || "", d.city || "", d.companyName || "",
    d.locationsServed || "", d.servicesOffered || "", d.category || "", d.keywords || "", d.teamSetup || "", d.targetClients || "",
    d.openForReferrals || "", d.referralTypes || "", d.websiteUrl || "", d.portfolioUrl || "", d.linkedinUrl || "", d.introPitch || ""
  ]);
}

// ==============================================================================
// GOOGLE DRIVE & DOC GENERATION (IN DOCUMENT FORMAT, NOT PDF)
// ==============================================================================
...
function formatDocHeader(body, subtitle, title, clientName) {
  ...
  pSub.setForegroundColor("#D97706"); // Amber gold
  pSub.setBold(true);
...
function sendNotificationToOwner(formType, data, brandName, clientName, clientEmail, docUrl, docxBlob) {
  ...
  if (docUrl) {
    htmlBody += `
      ...
      <p style="font-size: 13px; color: #6b7280;">
        <em>Note: The questionnaire has also been attached as a Microsoft Word (.docx) document (not PDF) to this email for your offline review and editing.</em>
      </p>
    `;
  }
  ...
  htmlBody += `
        ...
        <p style="font-size: 12px; color: #9ca3af; margin: 0;">
          All responses have been logged to the central Koiostudio Google Spreadsheet.
        </p>
      </div>
    </div>
  `;
